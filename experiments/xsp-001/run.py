...
CASE_ID="IC-XSP-001"
CASE_DIR=ROOT/"cases"/"xsp-001"
OUT=ROOT/"evidence"/"xsp-001"
OUT.mkdir(parents=True,exist_ok=True)

def evaluate(v):
    if v.get("proof") != "valid": return "deny-proof"
    if v.get("proof_purpose") != "assertionMethod": return "deny-proof-purpose"
    if v.get("issuer_authority") != "trusted-for-purpose": return "deny-issuer-authority"
    if v.get("credential_status") not in ("active","not-applicable"): return "deny-status"
    if v.get("audience") != "match": return "deny-audience"
    if v.get("freshness") != "fresh": return "deny-freshness"
    if v.get("relying_policy") != "allow": return "deny-policy"
    if not v.get("evidence_complete",False): return "deny-evidence"
    return "allow"
...
now=os.environ.get("EXECUTED_AT", "2026-08-18T01:10:46Z")
